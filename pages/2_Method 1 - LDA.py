...
def summarize_text_with_bart(topic_words, model, tokenizer):
    """Summarize topic words using BART."""
    ...
    if first_period_index != -1 and first_period_index + 1 < len(summary):
        return summary[first_period_index + 1:].strip()  # Exclude the first period and strip whitespace
    else:
        return summary  # If no period is found, return the full summary
...
# num_topics = st.slider("Select number of topics to be generated", min_value=2, max_value=10, value=3)
