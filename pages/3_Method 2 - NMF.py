...
/*Slider css*/
.stSlider .st-bq, .st-bb, .st-be, .st-bx, .st-br, .st-bs, .st-bt, .st-bu, .st-bv, .st-bw, .st-hx, .st-in, .st-ik, .st-il, .st-im, .st-ix, .st-iy, .st-iz, .st-j0{
  background-image: linear-gradient(to right, black, #2e1d4f, #1e0452, darkblue);
}
...
def get_top_words_for_topic(model, vectorizer, num_words=15):
...
# def clean_input_for_bart():
#   # Step 1: Tokenize the words
#   keywords = nmf_output.split()

#   # Step 2: Remove duplicates using a set
#   unique_keywords = list(set(keywords))

#   # # Step 3: Remove stopwords (common words)
#   # stop_words = set(stopwords.words('english'))
#   # cleaned_keywords = [word for word in unique_keywords if word.lower() not in stop_words]

#   # Step 4: Join the cleaned keywords into a string
#   cleaned_input = ", ".join(unique_keywords)
...
def summarize_text_with_bart(topic_words, model, tokenizer):
    ...
    # Generate summaries while passing the attention mask
    ...
    first_period_index = summary.find(".")
    if first_period_index != -1 and first_period_index + 1 < len(summary):
        return summary[first_period_index + 1:].strip()
    else:
        return summary
...
if process_button and uploaded_files:
    ...
    nmf_model = NMF(n_components=num_topics, random_state=1, init='random', max_iter = 500)
    nmf_model.fit(tfidf_matrix)

    # Topic Identified Section Container
    st.header("Topics Identified")
    for topic_label in assign_topic_labels(nmf_model, vectorizer):
        st.markdown(topic_label, unsafe_allow_html=True)

    # Topic Summary Section Container
    st.header("💬 Topic Summary")
    top_words_per_topic = get_top_words_for_topic(nmf_model, vectorizer)
    st.write(top_words_per_topic)
    #top_words = clean_input_for_bart()
    for topic_idx, top_words in enumerate(top_words_per_topic):
        summary = summarize_text_with_bart(top_words, bart_model, bart_tokenizer)
